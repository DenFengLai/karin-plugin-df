...
export const dir = {
  ...
  /** 插件配置目录 `@karinjs/karin-plugin-df/config` */
  get ConfigDir () {
    return path.join(this.karinPath, 'config')
  },
  /** 插件资源目录 `@karinjs/karin-plugin-df/resources` */
  get defResourcesDir () {
    return path.join(this.karinPath, 'resources')
  },
  /** 插件表情包存放目录 `@karinjs/karin-plugin-df/resources/face-images` */
  get faceResourcesDir () {
    return path.join(this.defResourcesDir, 'face-images')
  }
}
